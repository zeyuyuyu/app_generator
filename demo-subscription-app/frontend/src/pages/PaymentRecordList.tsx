...
export default function PaymentRecordList() {
  ...
  // 获取数据
  const { data: items = [], isLoading, error } = useQuery<PaymentRecord[]>(
    'paymentRecords',
    paymentRecordService.getAll
  )
  
  // 删除操作
  const deleteMutation = useMutation(paymentRecordService.delete, {
    onSuccess: () => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">管理所有支付记录</p>
        </div>
        <button
          onClick={() => {
            setEditingItem(null)
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-md">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">subscriptionId</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">amount</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">paymentDate</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredItems.map((item) => (
              <tr key={item.id}>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.subscriptionId}</td>
...
function PaymentRecordForm({ 
  ...
  const saveMutation = useMutation(
    item ? paymentRecordService.update : paymentRecordService.create,
    {
      onSuccess: onSave
    }
  )
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <h2 className="text-lg font-semibold mb-4">{item ? '编辑' : '新增'} 支付记录</h2>
        
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">subscriptionId</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">paymentDate</label>
            <input
              type="datetime-local"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">paymentStatus</label>
            <input
              type="text"
